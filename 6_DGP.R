...
lpriorGPint = function(alpha) {
  ...
  if (max(abs(alpha-0.5)) <1) { #The inner GP output should not
...
#Compute the DGP output at inputs x defined by phi matrix
#mphix, from values alphaint at the inner knots
#and alphaext at the outer knots
predDGP = function(mphix,alphaint,alphaext) {
  as.vector(phi(mphix%*%alphaint,m)%*%alphaext)
}

#Gaussian log likelihood function of the intputs given 
#alphaint and alphaext as above
llik = function(alphaint,alphaext) {
  alphaint = as.vector(alphaint)
  alphaext = matrix(nrow=m,ncol=1,data=alphaext)
  my = predDGP(Mn,alphaint,alphaext)
  dmvnorm(x=y,mu=my,Sigma=sdn^2*diag(n),log=TRUE)
}
...
for (i in 1:length(indKeepDGP)) {
  ...
  samplesfDGP[i,] = predDGP(Mt,alphaint,alphaext)
}
...
save(lpostDGP,nbatch,scale,outDGP,samplesAlphaDGP,indKeepDGP,
     samplesfDGP,mytDGP,predDGP,qitDGP,qstDGP,file=name)
